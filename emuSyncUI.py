...
from PyQt5.QtWidgets import (QApplication, QLineEdit, QToolButton, QCheckBox, QPushButton, QTableWidget, QMainWindow,
                             QFileDialog, QMessageBox, QTableWidgetItem, QAction, QMenu, QHeaderView, QRadioButton)
from PyQt5.QtGui import QIcon, QColor
from engineController import EngineController
from workspaceController import CPUState, CPUCommand
from configurationController import LogLevel
...
class MainUI(QMainWindow):
    def __init__(self, configurationFile):
        ...
        self.ui = uic.loadUi("emuSyncUI.ui", self)
        self.ui.installEventFilter(self)

        # Icon setup
        emuSyncIcon = QIcon()
        emuSyncIcon.addFile("res/EmuSync.ico")
        ...
        self.selectedInstance = self.ui.selectedInstanceComboBox
    ...
    def eventFilter(self, source, event):
        ...
        elif ((source is self.ui) and (event.type() == QEvent.KeyPress) and (event.key() == Qt.Key_F1) and (event.modifiers() == Qt.NoModifier)):
    ...
    def openConfClicked(self):
        ...
        configurationFilePathText = QFileDialog.getOpenFileName(self, "Select configuration file", '', "EmuSync Configuration File(*.EmuSync)")
...
def main(configurationPath='configurations.EmuSync'):
    ...
    Path(dirPath + r"/log/").mkdir(parents=True, exist_ok=True)
    logDirectory = 'log'
    if not os.path.isdir(logDirectory):
        os.makedirs(logDirectory)
    logPath = os.path.join(logDirectory, 'emuSyncUI.log')
